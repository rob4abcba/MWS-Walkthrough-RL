...
Add Watchify and Browserify, so we can use javascript bundles for each `html` file and one for the `service worker` to avoid having to add `<script>` tags every time we need to include another js file and allow us to use `import` and `npm packages` like Jake's `idb` for IndexedDB Promised.
